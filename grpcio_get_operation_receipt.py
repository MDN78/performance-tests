...
# 1. Формируем запрос на создание пользователя с рандомными данными
...
# Отправляем запрос и получаем ответ создания пользователя
create_user_response: CreateUserResponse = users_gateway_service.CreateUser(create_user_request)
print('Create user response:', create_user_response)

# 2. Формируем запрос на открытие дебетовой карты по ID, полученному из предыдущего ответа
